...
Window rotation is a doom default, but /layout/ rotation is not. This snippet adds =SPC w SPC= to rotate windows, and arrow key analogs of =hjkl= in the rare event I use them.
#+begin_src emacs-lisp
...
This i  s all about tweaking language specific packages or functionality to make these coding languages less miserable to use.
** File Types
#+begin_src emacs-lisp
...
**** \LaTeX Fragments
***** Highlighting
Do the sensible thing and highlight the fragment like you would in a ~*.tex~ file.
#+begin_src emacs-lisp
(setq org-highlight-latex-and-related '(native script entities))
#+end_src
This adds the ~org-block~ face however. To avoid this, it is neccessary to add yet another ~:inherit default~
#+begin_src emacs-lisp
(require 'org-src)
(add-to-list 'org-src-block-faces '("latex" (:inherit default :extend t)))
#+end_src
*****  Automatic Rendering
While editing the rest of the document, it would be nice to see the \LaTeX fragment as it will appear, which is done with the following package
#+begin_src emacs-lisp :tangle packages.el
...
#+end_src
***** Prettier Rendering
\LaTeX fragments by default won't fit the text around it, which causes many issues
#+begin_src emacs-lisp
...
Finally, make the background face transparent for the sake of it.
#+begin_src emacs-lisp
(setq org-format-latex-options
      (plist-put org-format-latex-options :background "Transparent"))
#+end_src
*** Export
Use Zotero's format styles in org-cite
#+begin_src emacs-lisp
(setq! org-cite-csl-styles-dir "~/Zotero/styles")
#+end_src

Make LaTeX and the gang support more than three header levels
#+begin_src emacs-lisp
(setq org-export-headline-levels 5)
#+end_src

It can be very useful to use subtrees to structure a document, but sometimes these trees shouldn't go in the final document. ~ox-extra~ adds a tag ~:ignore:~ for this exact purpose.
#+begin_src emacs-lisp
(require 'ox-extra)
...
**** \LaTeX
***** Compiling
org's export process for LaTeX is terribly inefficient.
#+begin_src emacs-lisp
(setq org-latex-pdf-process '("latexmk -f -pdf -%latex -shell-escape -interaction=nonstopmode -output-directory=%o %f"))
#+end_src
***** Class Templates
Org only sets up one LaTeX class, =article=. This will not do
#+begin_src emacs-lisp
(after! ox-latex
...
AucTeX messes with the order of pdf-viewers
#+begin_src emacs-lisp
(setq! +latex-viewers '(pdf-tools zathura okular skim))
#+end_src
** Graphviz
#+begin_src emacs-lisp
